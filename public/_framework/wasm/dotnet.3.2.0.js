...
var MONO = {
  ...
  string_decoder: {
    copy: function (mono_string) {
      ...
      if (!this.mono_wasm_string_convert)
        this.mono_wasm_string_convert = Module.cwrap(
          "mono_wasm_string_convert",
          null,
          ["number"],
        );
    ...
    if (this._async_method_objectId != 0) {
      for (let i in res) {
        if (res[i].value.isValueType != undefined && res[i].value.isValueType)
          res[i].value.objectId =
            `dotnet:valuetype:${this._async_method_objectId}:${res[i].fieldOffset}`;
      }
    }
  ...
  mono_wasm_get_object_properties: function (objId, expandValueTypes) {
    ...
    var res = MONO._filter_automatic_properties(
      MONO._fixup_name_value_objects(this.var_info),
    );
    for (var i = 0; i < res.length; i++) {
      if (res[i].value.isValueType != undefined && res[i].value.isValueType)
        res[i].value.objectId =
          `dotnet:valuetype:${objId}:${res[i].fieldOffset}`;
    }
    this.var_info = [];
    return res;
  },
  ...
  mono_wasm_get_array_value_expanded: function (objId, idx) {
    if (!this.mono_wasm_get_array_value_expanded_info)
      this.mono_wasm_get_array_value_expanded_info = Module.cwrap(
        "mono_wasm_get_array_value_expanded",
        null,
        ["number", "number"],
      );
    this.var_info = [];
  ...
  _is_object_id_array: function (objectId) {
    return (
      objectId.startsWith("dotnet:array:") && objectId.split(":").length == 3
    );
  },
  _get_array_details: function (objectId, objectIdParts) {
    switch (objectIdParts.length) {
      case 3:
        return this._post_process_details(
          this.mono_wasm_get_array_values(objectIdParts[2]),
        );
      case 4:
        var arrayObjectId = objectIdParts[2];
        var arrayIdx = objectIdParts[3];
        return this._get_details_for_value_type(objectId, () =>
          this.mono_wasm_get_array_value_expanded(arrayObjectId, arrayIdx),
        );
      default:
  ...
  mono_wasm_get_details: function (objectId, args) {
    ...
    if (parts[0] != "dotnet")
      throw new Error(
        "Can't handle non-dotnet object ids. ObjectId: " + objectId,
      );
    switch (parts[1]) {
      case "object":
        if (parts.length != 3)
          throw new Error(
            `exception this time: Invalid object id format: ${objectId}`,
          );
        return this._post_process_details(
          this.mono_wasm_get_object_properties(parts[2], false),
        );
      case "array":
      ...
      case "valuetype":
        ...
        return this._get_details_for_value_type(objectId, () =>
          this.mono_wasm_get_object_properties(containerObjectId, true),
        );
      case "cfo_res": {
  ...
  mono_wasm_call_function_on: function (request) {
    ...
    var fn_args =
      request.arguments != undefined
        ? request.arguments.map((a) => a.value)
        : [];
  ...
  mono_wasm_add_null_var: function (className) {
    fixed_class_name = MONO._mono_csharp_fixup_class_name(
      Module.UTF8ToString(className),
    );
    if (!fixed_class_name) {
  ...
  _mono_wasm_add_string_var: function (var_value) {
    ...
    MONO.var_info.push({ value: { type: "string", value: var_value } });
  },
  _mono_wasm_add_getter_var: function (className) {
    ...
    MONO.var_info.push({
      value: { type: "symbol", value: value, description: value },
    });
  },
  ...
  mono_wasm_add_typed_value: function (type, str_value, value) {
    ...
    switch (type_str) {
      case "bool":
        MONO.var_info.push({ value: { type: "boolean", value: value != 0 } });
        break;
      case "char":
        MONO.var_info.push({
          value: {
        ...
        break;
      case "number":
        MONO.var_info.push({ value: { type: "number", value: value } });
        break;
      case "string":
      ...
      case "pointer":
        {
          MONO.var_info.push({
            value: { type: "symbol", value: str_value, description: str_value },
          });
        }
        break;
      default: {
        ...
        MONO.var_info.push({
          value: { type: "symbol", value: msg, description: msg },
        });
        break;
      }
    }
...
function _mono_set_timeout(timeout, id) {
  if (!this.mono_set_timeout_exec)
    this.mono_set_timeout_exec = Module.cwrap("mono_set_timeout_exec", null, [
      "number",
    ]);
...
function _mono_wasm_add_func_var(className, targetName, objectId) {
  ...
  var type_name = MONO._mono_csharp_fixup_class_name(
    Module.UTF8ToString(className),
  );
  if (objectId == -1) {
    MONO.var_info.push({
      value: { type: "symbol", value: tgt_sig, description: tgt_sig },
    });
  } else {
    MONO.var_info.push({
      value: {
...
var BINDING = {
  ...
  bindings_lazy_init: function () {
    ...
    var get_method = function (method_name) {
      ...
      if (!res)
        throw (
          "Can't find method " + namespace + "." + classname + ":" + method_name
        );
      return res;
    };
  ...
  typedarray_copy_to: function (
    ...
    if (!!(
      typed_array.buffer instanceof ArrayBuffer && typed_array.BYTES_PER_ELEMENT
    )) {
  ...
  call_method: function (method, this_arg, args_marshal, args) {
    ...
    if (has_args_marshal && has_args) {
      ...
      if (!converters) {
        ...
        converters.set("s", {
          steps: [{ convert: this.js_string_to_mono_string.bind(this) }],
          size: 0,
        });
        converters.set("o", {
          steps: [{ convert: this.js_to_mono_obj.bind(this) }],
          size: 0,
        });
        converters.set("u", {
          steps: [{ convert: this.js_to_mono_uri.bind(this) }],
          size: 0,
        });
        converters.set("k", {
          steps: [
            { convert: this.js_to_mono_enum.bind(this), indirect: "i64" },
          ],
          size: 8,
        });
        converters.set("j", {
          steps: [
            { convert: this.js_to_mono_enum.bind(this), indirect: "i32" },
          ],
          size: 8,
        });
      ...
      for (i = 0; i < args.length; ++i) {
        ...
        var obj = handler.convert
          ? handler.convert(args[i], method, i)
          : args[i];
        if (handler.indirect) {
...
var _mono_wasm_assembly_find_class = (Module["_mono_wasm_assembly_find_class"] =
  function () {
...
var _mono_wasm_string_convert = (Module["_mono_wasm_string_convert"] =
  function () {
    return (_mono_wasm_string_convert = Module["_mono_wasm_string_convert"] =
...
var _malloc = (Module["_malloc"] = function () {
  return (_malloc = Module["_malloc"] = Module["asm"]["Yb"]).apply(
    null,
    arguments,
  );
});
var _memset = (Module["_memset"] = function () {
  return (_memset = Module["_memset"] = Module["asm"]["Zb"]).apply(
    null,
    arguments,
  );
});
...
var _putchar = (Module["_putchar"] = function () {
  return (_putchar = Module["_putchar"] = Module["asm"]["ac"]).apply(
    null,
    arguments,
  );
});
...
var _mono_wasm_get_array_value_expanded = (Module[
  "_mono_wasm_get_array_value_expanded"
] = function () {
  return (_mono_wasm_get_array_value_expanded = Module[
    "_mono_wasm_get_array_value_expanded"
  ] =
...
var _htonl = (Module["_htonl"] = function () {
  return (_htonl = Module["_htonl"] = Module["asm"]["mc"]).apply(
    null,
    arguments,
  );
});
var _htons = (Module["_htons"] = function () {
  return (_htons = Module["_htons"] = Module["asm"]["nc"]).apply(
    null,
    arguments,
  );
});
var _ntohs = (Module["_ntohs"] = function () {
  return (_ntohs = Module["_ntohs"] = Module["asm"]["oc"]).apply(
    null,
    arguments,
  );
});
...
var __get_tzname = (Module["__get_tzname"] = function () {
  return (__get_tzname = Module["__get_tzname"] = Module["asm"]["qc"]).apply(
    null,
    arguments,
  );
});
...
var _setThrew = (Module["_setThrew"] = function () {
  return (_setThrew = Module["_setThrew"] = Module["asm"]["tc"]).apply(
    null,
    arguments,
  );
});
...
var _memalign = (Module["_memalign"] = function () {
  return (_memalign = Module["_memalign"] = Module["asm"]["xc"]).apply(
    null,
    arguments,
  );
});
var dynCall_vi = (Module["dynCall_vi"] = function () {
  return (dynCall_vi = Module["dynCall_vi"] = Module["asm"]["yc"]).apply(
    null,
    arguments,
  );
});
...
var stackAlloc = (Module["stackAlloc"] = function () {
  return (stackAlloc = Module["stackAlloc"] = Module["asm"]["Ac"]).apply(
    null,
    arguments,
  );
});
var stackRestore = (Module["stackRestore"] = function () {
  return (stackRestore = Module["stackRestore"] = Module["asm"]["Bc"]).apply(
    null,
    arguments,
  );
});
var dynCall_v = (Module["dynCall_v"] = function () {
  return (dynCall_v = Module["dynCall_v"] = Module["asm"]["Cc"]).apply(
    null,
    arguments,
  );
});
var dynCall_ii = (Module["dynCall_ii"] = function () {
  return (dynCall_ii = Module["dynCall_ii"] = Module["asm"]["Dc"]).apply(
    null,
    arguments,
  );
});
