...
import { LastStudyCard } from "@/components/last-study-card";
...
const accentStyles = {
  ...
  orange: {
    badge: "bg-orange-400/10 text-orange-300 ring-orange-400/20",
    glow: "group-hover:border-orange-400/50",
  },
  violet: {
    badge: "bg-violet-400/10 text-violet-300 ring-violet-400/20",
    glow: "group-hover:border-violet-400/50",
  },
  emerald: {
  ...
  indigo: {
    badge: "bg-indigo-400/10 text-indigo-300 ring-indigo-400/20",
          ...
          <div className="mb-6 flex items-end justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                {languages.length}개 과정 · 총{" "}
                {languages.reduce(
                  (total, language) => total + language.courseLength,
                  0,
                )}
                개 학습
              </p>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between">
                    <span
                      className={`grid size-14 place-items-center rounded-2xl font-mono text-lg font-black ring-1 ${styles.badge}`}
                    >
                      {language.shortName}
                    </span>
                    <span
                      className="text-2xl text-slate-600 transition group-hover:translate-x-1 group-hover:text-white"
                      aria-hidden
                    >
                      →
                    </span>
                  ...
                  <p className="mt-2 grow text-sm leading-6 text-slate-400">
                    {language.description}
                  </p>
                  <div className="mt-6 flex items-center gap-3 text-xs font-medium text-slate-500">
                    <span>{language.courseLength}일 코스</span>
                    <span>·</span>
                    <span>심화 60 + 프로젝트 60</span>
                  </div>
                </Link>
              );
            })}
