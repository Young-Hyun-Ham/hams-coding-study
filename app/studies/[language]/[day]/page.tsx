...
export async function generateMetadata({
  ...
  const lesson = language
    ? getLesson(language.slug, Number(dayParam))
    : undefined;
  return lesson && language
    ? {
        title: `Day ${lesson.day}. ${lesson.title} | ${language.name} 스터디`,
...
export default async function LessonPage({ params }: PageProps) {
  ...
  const lesson =
    language && Number.isInteger(day)
      ? getLesson(language.slug, day)
      : undefined;
  if (!language || !lesson) notFound();

  const requiresLogin =
    lesson.level === "advanced" || lesson.level === "project";
  if (requiresLogin) {
    const requestHeaders = await headers();
    const authRequest = new Request("http://hcs.local", {
      headers: { cookie: requestHeaders.get("cookie") ?? "" },
    });

    if (!getSsoUserFromRequest(authRequest)) {
      const lessonPath = `/studies/${language.slug}/${lesson.day}`;
      redirect(
        `/studies/${language.slug}?loginRequired=${encodeURIComponent(lessonPath)}`,
      );
    }
  }
      ...
      <div className="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8">
        <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div className="flex min-w-0 items-center gap-3 text-sm">
            ...
            >
              {language.name} {language.courseLength}일 코스
            </Link>
            <span className="text-slate-700">/</span>
            ...
            <section className="mt-7 rounded-xl bg-slate-950 p-5 ring-1 ring-slate-800">
              ...
              <details className="group mt-4 overflow-hidden rounded-lg border border-slate-800 bg-slate-900/70">
                <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3 text-sm font-bold text-sky-300 marker:content-none">
                  <span>
                    <strong>{lesson.title}</strong> 자세히 알아보기
                  </span>
                  <span
                    className="text-lg text-slate-500 transition group-open:rotate-45"
                    aria-hidden
                  >
                    +
                  </span>
                ...
                <div className="border-t border-slate-800 px-4 py-4">
                  ...
                  <h3 className="mt-5 text-xs font-bold tracking-wider text-slate-300 uppercase">
                    핵심 포인트
                  </h3>
              ...
              <details className="group mt-3 overflow-hidden rounded-lg border border-slate-800 bg-slate-900/70">
                ...
                <div className="border-t border-slate-800 p-3">
                  <p className="mb-3 text-xs leading-5 text-amber-300/80">
                    먼저 직접 풀어본 다음 예시 풀이와 비교해 보세요.
                  </p>
                  <FormattedCode
                    language={language.slug}
                    code={lesson.practice.solutionCode}
                  />
                </div>
              </details>
            </section>
            ...
            {language.slug !== "python" &&
              ...
              language.slug !== "csharp" && (
                <p className="mt-5 rounded-lg border border-amber-400/20 bg-amber-400/5 p-3 text-xs leading-5 text-amber-200/80">
                  현재 브라우저 코드 실행은 순수 Python/Pyodide 과정에서
                  지원합니다. LangGraph를 포함한 외부 런타임 과정은 Monaco 코드
                  작성과 자동 저장 상태를 제공합니다.
                </p>
              )}
          </aside>
        ...
        <nav
          ...
          aria-label="이전 및 다음 학습"
        >
          {previous ? (
            <Link
              href={`/studies/${language.slug}/${previous.day}`}
              className="rounded-xl border border-slate-800 bg-slate-900 p-4 transition hover:border-slate-700"
            >
              <span className="text-xs text-slate-500">← 이전 학습</span>
              <strong className="mt-1 block text-sm">
                Day {previous.day}. {previous.title}
              </strong>
            </Link>
          ) : (
            <div />
          ...
          {next ? (
            <Link
              href={`/studies/${language.slug}/${next.day}`}
              className="rounded-xl border border-slate-800 bg-slate-900 p-4 text-right transition hover:border-sky-500/40"
            >
              <span className="text-xs text-slate-500">다음 학습 →</span>
              <strong className="mt-1 block text-sm">
            ...
            <Link
              href={`/studies/${language.slug}`}
              className="rounded-xl border border-emerald-400/20 bg-emerald-400/5 p-4 text-right text-emerald-300"
            >
              {language.courseLength}일 과정 완료 · 목록으로 →
            </Link>
          )}
        </nav>
      </div>
