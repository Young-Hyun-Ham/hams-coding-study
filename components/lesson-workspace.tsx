...
import { capitalize, getLessonFileName, getWorkerName } from "@/lib/utils";
...
export function LessonWorkspace({
  ...
  const stdin = useLessonWorkspaceStore(
    (state) => state.stdinByLesson[lessonId] ?? "",
  );
  ...
  const extension =
    language === "python" || language === "python-langgraph"
      ? "py"
      : language === "java"
        ? "java"
        : language === "kotlin"
          ...
          : language === "c"
            ...
            : language === "csharp"
              ...
              : language === "javascript"
                ? "js"
                : "tsx";
  const fullFileName = `${lessonId}.${extension}`;
  const acceptsStdin =
    language === "python" || language === "javascript" || language === "c";
  const execution = useMutation({
    mutationFn: (input: { code: string; stdin: string }) =>
      runBrowserCode({ language, ...input }),
  });
  ...
  return (
    <section className="min-w-0 max-w-full overflow-hidden rounded-2xl border border-slate-800 bg-slate-900">
      ...
      <header className="flex items-center gap-2 overflow-x-auto whitespace-nowrap border-b border-slate-800 px-3 py-2">
        <div className="flex shrink-0 items-center gap-2">
          ...
          <span
            ...
            title={fullFileName}
          >
            {getLessonFileName(lessonId, extension)}
          </span>
        </div>
        <div className="ml-auto flex shrink-0 flex-nowrap justify-end gap-1.5">
          <button
            ...
            title={
              canExecute
                ? "코드 실행"
                : "현재 Python 과정만 브라우저 실행을 지원합니다."
            }
            className="rounded-md bg-sky-400 px-3 py-1.5 text-xs font-bold text-slate-950 transition hover:bg-sky-300 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400"
          >
            {execution.isPending
              ? "실행 중..."
              : canExecute
          ...
          <button
            ...
            className="rounded-md border border-amber-400/30 px-2 py-1.5 text-xs text-amber-300 transition hover:bg-amber-400/10"
          >
          ...
          <CloudLessonControls
            ...
            onLoad={(content) => {
              ...
              setStdin(lessonId, content.stdin);
              execution.reset();
            }}
          />
      ...
      <div className="grid border-t border-slate-800 md:grid-cols-2">
        <div className="border-b border-slate-800 p-4 md:border-r md:border-b-0">
          ...
          <textarea
            ...
            placeholder={
              acceptsStdin
                ? "표준 입력으로 읽을 값을 줄 단위로 입력하세요."
                : "이 실행 환경에서는 표준 입력을 사용하지 않습니다."
            }
        ...
        <div className="p-4" aria-live="polite">
          ...
          <pre
            className={`h-28 overflow-auto whitespace-pre-wrap rounded-lg bg-slate-950 p-3 text-sm ${execution.isError || execution.data?.stderr ? "text-rose-300" : "text-slate-300"}`}
          >
            {output}
          </pre>
        </div>
      ...
      <CommonModal
        ...
        description="코드 실행은 HAMS 계정으로 로그인한 사용자만 이용할 수 있습니다."
        prompt="로그인 페이지로 이동하시겠습니까?"
        confirmHref={`/login?returnTo=${encodeURIComponent(`/studies/${language}/${day}`)}`}
      />
    </section>
  );
}
