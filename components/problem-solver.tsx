...
export function ProblemSolver() {
  ...
  const execution = useMutation({ mutationFn: (input: { code: string; stdin: string }) => runPython(input) });
  const handleEditorMount: OnMount = (editor) => editor.focus();
    ...
    <section className="grid min-h-[680px] overflow-hidden rounded-2xl border border-slate-800 bg-slate-900 shadow-2xl shadow-sky-950/30 lg:grid-cols-[minmax(280px,0.7fr)_minmax(0,1.3fr)]">
      <article className="border-b border-slate-800 p-6 lg:border-r lg:border-b-0 lg:p-8">
        <div className="mb-6 flex items-center gap-2 text-xs font-semibold tracking-[0.18em] text-sky-400 uppercase">
          <span className="size-2 rounded-full bg-sky-400" /> Python · Pyodide · Easy
        </div>
        <h1 className="text-2xl font-bold text-white">배열의 합 구하기</h1>
        <p className="mt-4 leading-7 text-slate-300">
          정수 배열 <code className="rounded bg-slate-800 px-1.5 py-0.5 text-sky-300">numbers</code>가
          주어질 때 모든 원소의 합을 반환하세요.
        </p>
      ...
      <div className="flex min-w-0 flex-col">
        ...
        <div className="grid border-t border-slate-800 md:grid-cols-2">
          <label className="border-b border-slate-800 p-4 md:border-r md:border-b-0">
            <span className="mb-2 block text-xs font-semibold text-slate-500 uppercase">표준 입력 (stdin)</span>
            <textarea
              value={stdin}
          ...
          <div className="p-4" aria-live="polite">
            <div className="mb-2 flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-500 uppercase">실행 결과</span>
              {execution.data && (
                <span className={execution.data.exitCode === 0 ? "text-xs text-emerald-400" : "text-xs text-rose-400"}>
                  exit {execution.data.exitCode}
                </span>
              )}
            </div>
            <pre className={`h-28 overflow-auto whitespace-pre-wrap rounded-lg bg-slate-950 p-3 text-sm ${execution.isError || execution.data?.stderr ? "text-rose-300" : "text-slate-200"}`}>
              {output}
            </pre>
          </div>
        </div>
