...
import {
  langGraphExplanations,
  solutionCatalog,
} from "./curriculum-solutions.mjs";
import { makeDetailedExplanation } from "./curriculum-explanations.mjs";
import {
  makeAdvancedTopics,
  makeProjectTopics,
} from "./curriculum-extension.mjs";
...
const curricula = {
  ...
  "python-langgraph": [
    ...
    "Overwrite로 값 교체",
    "비동기 노드",
    "RetryPolicy와 CachePolicy",
    "미니 프로젝트: 요청 분류 그래프",
    "조건부 엣지",
    "Literal 기반 라우팅",
    "Command로 업데이트와 이동",
    "Send를 이용한 Map-Reduce",
    "순환 그래프와 에이전트 루프",
    "재귀 제한과 무한 루프 방지",
    "병렬 브랜치 실행",
    "병렬 업데이트 리듀서",
...
for (const [language, topics] of Object.entries(curricula)) {
  ...
  if (expandedTopics.length !== 160)
    throw new Error(`${language} 과정은 정확히 160개여야 합니다.`);
  ...
  const lessons = expandedTopics.map((title, index) => {
    ...
    const practice = {
      ...generatedPractice,
      solutionCode:
        index < 40 && solutionCatalog[language]
...
console.log(
  `${Object.keys(curricula).length}개 과정, 총 ${Object.keys(curricula).length * 160}개 학습 데이터를 생성했습니다.`,
);
